// auth.js - Minified authentication module
...
async function fetchUserProfile() {
  try {
    const response = await fetchWithAuth(
      `${CONFIG.API_BASE_URL}${CONFIG.API_ENDPOINTS.USER_DETAILS}`,
      { method: "GET" },
    );
    if (response.ok) {
...
async function updateUserProfile(userData) {
  try {
    const response = await fetchWithAuth(
      `${CONFIG.API_BASE_URL}${CONFIG.API_ENDPOINTS.USER_UPDATE}`,
...
async function refreshAccessToken() {
  ...
  try {
    const response = await fetch(
      `${CONFIG.API_BASE_URL}${CONFIG.API_ENDPOINTS.REFRESH}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function showToast(message, type = "info") {
  ...
  toast.style.cssText = `position:fixed;bottom:20px;right:20px;padding:12px 20px;background:${type === "success" ? "#4CAF50" : type === "error" ? "#f44336" : "#2196F3"};color:white;border-radius:8px;z-index:10000;animation:slideIn 0.3s ease;`;
